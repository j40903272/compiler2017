...
void genBlock(AST_NODE *blockNode){
	...
	fprintf(output, ".data\n");
	fprintf(output, "_OLD_SP_%d: .skip 8\n", id);
	fprintf(output, ".text\n");
	fprintf(output, "ldr x%d, =_OLD_SP_%d\n", reg1, id);
...
void genAssignStmt(AST_NODE *assignNode){
	...
	if(idNode->semantic_value.identifierSemanticValue.kind == NORMAL_ID){
		...
		for(AST_NODE *dimListNode = idNode->child ; dimListNode != NULL ; dimListNode = dimListNode->rightSibling){
			...
			int reg2 = get_reg();
			fprintf(output, ".data\n");
			fprintf(output, "_const_%d: .word %d\n", g_cnt, entry->attribute->attr.typeDescriptor->properties.arrayProperties.sizeInEachDimension[dim]);
			gen_Alignment();
			fprintf(output, ".text\n");
			fprintf(output, "ldr x%d, _const_%d\n", reg2, g_cnt++);
